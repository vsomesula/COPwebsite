...
pacman::p_load(pacman,readxl, magrittr, productplots, psych, RColorBrewer, tidyverse)
...
```
# Explaotory Analysis
## This type of model will give us an equation where we can use as many of the categorial variables as we want. We can imagine it as CutGx1 + CutIx2 + .. with their respective coefficents. With each duiamond if it has Cut level of G than x2, x3, etc for that category will be 0 leaving us only with the weight for Cut G. This will leave us a very convuluted model however as seen below. We will remove wholesaler as it is not mentioned by the professor
```{r}
